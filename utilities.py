...
def crop_image_center(img: Image.Image, M: np.ndarray) -> Image.Image:
    """
    Crop une image pour garder l'objet au milieu de l'image
    Prend en paramètre l'image à crop ainsi que son avant-plan
    """
    try:
        len(M) > 0
    except ValueError:
        print("Désolé l'image est vide")

    # On détermine où se trouve l'objet sur l'image
    minx, miny, maxx, maxy = len(M), len(M[0]), 0, 0
    ...
    # On crop l'image selon les bornes de l'objet
    cropped_img = img.crop((minx, miny, maxx, maxy))
    # if maxx - minx > maxy - miny:
    # taille = maxx - minx
    # else:
    # taille = maxy - miny
    # cropped_img = crop_around_center(img, center, taille)
    return cropped_img
...
def foreground_darkblue_red_filter(
    img: Union[Image.Image, np.ndarray]
) -> np.ndarray:
    """Extrait l'avant-plan bleu foncé ou rouge d'une image"""
    M = np.array(img)
    R = M[:, :, 0] * 1.0
    G = M[:, :, 1] * 1.0
    B = M[:, :, 2] * 1.0
...
def foreground_white_filter(
    img: Union[Image.Image, np.ndarray]
) -> np.ndarray:
    """Extrait l'avant-plan blanc d'une image"""
    M = np.array(img)
    R = M[:, :, 0] * 1.0
    G = M[:, :, 1] * 1.0
    B = M[:, :, 2] * 1.0
...
def symetrie(img: Image.Image) -> bool:
    """
    Vérifie la symétrie verticale d'une image entre la partie gauche et la partie droite
    en terme de couleur moyenne.

    Une couleur moyenne assez proche (<35) dans chacune des parties suggère un 0,
    une grosse différence (>35) suggère un 1.
    """
    M = np.array(img)
    # Sépare l'image en deux, la partie gauche et la partie droite
    gauche = M[:, img.width//2:, :] * 1.0
    droite = M[:, :img.width//2, :] * 1.0
    return abs(np.mean(gauche) - np.mean(droite))


def trou(img: Image.Image) -> bool:
    """
    Vérifie la présence d'un trou dans une image (schéma comprenant un ou des pixels
    noirs, puis un ou des pixels blancs, puis à nouveau un ou des pixels noirs.

    Renvoie 1 si oui, 0 sinon. 1 suggère un 0, 0 suggère un 1.
    """
    M = np.array(img)
    # Récupère la ligne centrale de l'image
    moitie = M[img.height // 2, :, :]
    pixel_noir, trou, pixel_blanc = False, False, False
    for pixel in moitie:
        # Si on croise un pixel noir pour la première fois
        if pixel.mean() == 63.75 and not pixel_noir:
            pixel_noir = True
            pixel_blanc = False
        # Si on croise un pixel noir après avoir eu des pixels noirs puis blancs
        elif pixel.mean() == 63.75 and pixel_noir and pixel_blanc:
            trou = True
            break
        # Si on croise un pixel blanc
        elif pixel.mean() == 255:
            pixel_blanc = True
    return int(trou)


def ratio_bleu_rouge(img: Image.Image) -> float:
    bleu = blueness(img)
    rouge = redness(img)
    return bleu/rouge


def ratio_perimetre_surface(img: Image.Image) -> float:
    perimetre = perimeter(img)
    surf = surface(img)
    return perimetre/surf
