...
Chaque image fait 32x32 pixels, et chaque pixel contient 3 composantes RGB, donc on obtient 32x32x3 colonnes pour chaque image, soit 3072 colonnes (0 à 3071)
...
Ces données ne sont pas interprétables ainsi, la couleur moyenne d'une composante d'une rangée de pixels pour chaque image ne donne aucune information, de même pour l'écart-type, le minimum etc...
...
Oui car le résultat est relativement bon alors que l'on a crop les images sans même s'intéresser à centrer l'image sur l'objet à détecter
...
---
import sys
df_scaled = (df.drop('étiquette', axis=1) - df.drop('étiquette', axis=1).mean()) / (df.drop('étiquette', axis=1).std() + sys.float_info.epsilon)
```
...
nbgrader:
  ...
  task: false
---
valeurs_propres = S**2 / np.sum(S**2)

plt.plot(valeurs_propres[:50])
plt.show()
```
...
nbgrader:
  ...
  task: false
---
svd_df = pd.DataFrame(U[: , :5], index=df.index) 
svd_df.insert(5, "étiquette", df["étiquette"])
svd_df
```
...
if dataset_dir == os.path.join(data.dir, 'data'):
    assert svd_df.shape == (30,6) 
```
...
Non, les données sont mélangées entre elles et rien de permet à vue d'oeil de les différencier
...
Les performances sont moins bonnes qu'avec la représentation directe des pixels, la différence n'est pas énorme mais tout de même notable
...
for i in range(1, 11):
    svd_df = pd.DataFrame(U[: , :i], index=df.index) 
    svd_df.insert(i, "étiquette", df["étiquette"])
    p_tr, s_tr, p_te, s_te = df_cross_validate(svd_df, sklearn_model, sklearn_metric, verbose=True)
    attributs.append(i)
    scores.append(p_te)
    
plt.plot(attributs, scores)
plt.show()
```

+++ {"deletable": false, "nbgrader": {"cell_type": "markdown", "checksum": "ad7aaa7d31ca9544864f9b28bbb9cb7c", "grade": true, "grade_id": "cell-c5a8e3d90bb35375", "locked": false, "points": 0, "schema_version": 3, "solution": true, "task": false}, "user_expressions": []}
...
On a vu que l'on pouvait classer les images avec une précision relativement bonne mais largement améliorable avec la simple analyse des composantes des pixels, mais cela n'est pas satisfaisant du tout et nous pouvons faire beaucoup plus efficace et rigoureux.
