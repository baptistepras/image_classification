...
```{code-cell} ipython3
show_source(foreground_darkblue_red_filter)
```

```{code-cell} ipython3
# image_grid([foreground_redness_filter(img, theta=0.6)
            # for img in sample])
    
image_grid([foreground_darkblue_red_filter(img)
            for img in sample])
```

```{code-cell} ipython3
# image_grid([foreground_white_filter(img)
            # for img in sample])
```
...
```{code-cell} ipython3
# image_grid([invert_if_light_background(foreground_redness_filter(img, theta=.4))
            # for img in sample])
```
...
# image_grid([scipy.ndimage.gaussian_filter(foreground_darkblue_red_filter(img), sigma=.0) for img in sample])
```
...
def my_foreground_filter(img):
    foreground = foreground_darkblue_red_filter(img)
    return foreground
```
...
+++ {"user_expressions": [], "jp-MarkdownHeadingCollapsed": true}
...
```{code-cell} ipython3
show_source(crop_image_center)
```

```{code-cell} ipython3
crop_image_center(img, my_foreground_filter(img))
```
...
def my_preprocessing(img):
    ...
    foreground = my_foreground_filter(img)
    img = transparent_background(img, foreground)
    img = crop_image_center(img, foreground)
    return img
```
...
df_clean = df_clean.fillna(0)  # car les images ne font pas la même taille
...
Les zones claires sont les zones avec une forte proportion de rouge, alors que les zones sombres avec une très faible proportion de rouge. Le fond n'est pas toujours de la même couleur car il n'a pas toujours la même couleur, sur certaines images il peut être plus ou moins rouge et avoir autant de rouge que le fruit en question, ce qui le confond avec le fruit
...
df_features = pd.DataFrame({'redness': clean_images.apply(redness),
                            ...
                            'symetrie': clean_images.apply(symetrie),
                            'trou': clean_images.apply(trou)})
"""
df_features = pd.DataFrame({'redness': clean_images.apply(redness),
                            'blueness': clean_images.apply(blueness),
                            'perimeter': clean_images.apply(perimeter),
                            ...
                            'symetrie': clean_images.apply(symetrie),
                            'trou': clean_images.apply(trou),
                            'ratioBR': clean_images.apply(ratio_bleu_rouge),
                            'ratioPS': clean_images.apply(ratio_perimetre_surface)})
df_features
```

```{code-cell} ipython3
# Tests pour savoir s'il existe un ratio entre le périmètre et la surface pour classifier nos images
p = [i for i in df_features["perimeter"]]
s = [i for i in df_features["surface"]]
r1, r2 = [], []
for i in range(15):
    r1.append(p[i]/s[i])
...
```{code-cell} ipython3
show_source(ratio_bleu_rouge)
```

```{code-cell} ipython3
show_source(ratio_perimetre_surface)
```
...
```{code-cell} ipython3
performances.loc[2] = ["8 attributs ad-hoc", p_tr, s_tr, p_te, s_te]
performances.style.format(precision=2).background_gradient(cmap='Blues')
```
...
Pommes et bananes: Il faudrait garder les 17 premiers attributs car c'est là où le score de performance est le meilleur                                                                    
Sens uniques/interdits: Il faudrait garder les 5 premiers attributs
...
```{code-cell} ipython3
performances.loc[3] = ["5 attributs par analyse de variance univarié", p_tr, s_tr, p_te, s_te]
performances.style.format(precision=2).background_gradient(cmap='Blues')
```
...
print("AVERAGE TEST {0:s} +- STD: {1:.2f} +- {2:.2f}".format(metric_name, p_te, s_te))
performances.loc[4] = ["Meilleure paire (blueness/redness)", p_tr, s_tr, p_te, s_te]
performances.style.format(precision=2).background_gradient(cmap='Blues')
```
...
Pommes et bananes: la meilleure paire est R | R-(G+B)/2, mais l'approche multivariée n'est pas nécessaire pour ce jeu, le résultat étant sensiblement moins bon                            
Sens uniques/interdits: blueness/redness est la meilleure paire, et c'est avec 5 attributs qu'on a le meilleur résultat
...
Jeu Sens Interdit/Unique:
On a pré-traité de deux manières notre jeu d'image (images carrées et pas carrées) en tentant de centrer l'objet et d'extraire au mieux la panneau de son fond (ville, ciel, arbres, fond blanc), et classé selon les attributs ad-hoc connus, en ayant essayé d'en implémenter d'autres, obtenant ainsi de très bon résultats
